        ...
        <form @submit.prevent="handleLogin" class="px-6 sm:px-8 py-8">
          ...
          <div class="grid grid-cols-2 gap-3">
            ...
            <button
              ...
              class="flex items-center justify-center py-2.5 px-3 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-700 dark:text-gray-300 font-semibold text-sm hover:bg-gray-50 dark:hover:bg-gray-800 transition-all duration-300"
            >
...
import { useAuthStore } from '@/stores/auth' // 1. Thêm Store của Pinia
...
const handleLogin = async () => {
  ...
  try {
    ...
    if (data.success) {
      // 3. THAY THẾ: Gọi Pinia Store để lưu Token + User một cách reactive trên toàn hệ thống
      // (Bên trong store đã bao gồm việc tự lưu vào localStorage cho bạn rồi)
      authStore.loginSuccess(data.data.user, data.data.token)
      
      // Lưu tùy chọn ghi nhớ đăng nhập nếu cần
...
const handleSocialLogin = (provider) => {
  ...
  alert(`Đăng nhập bằng ${provider} (Chức năng demo)`)
}
